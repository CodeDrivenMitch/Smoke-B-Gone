...
public class MenuHandler implements OnClickListener {
	...
	public static final int[] allMenuItems = { R.id.bmHome, R.id.bmInfo,
			R.id.bmSettings, R.id.bmHealth, R.id.bmChart, R.id.bmMoney };
	public static final String[] allMenuActions = { "MainActivity",
			"InfoActivity", "SettingsActivity", "HealthActivity",
			"XYChartBuilder", "MoneyActivity" };
	...
	public MenuHandler(Activity activity) {
		...
		for (int i = 0; i < MenuHandler.allMenuItems.length; i++) {
			activity.findViewById(MenuHandler.allMenuItems[i])
	...
	public static void launchClass(Activity motherActivity, String selectedClass) {
		...
		try {
			launchingClass = Class.forName("com.goodhearted.smokebegone."
					+ selectedClass);
